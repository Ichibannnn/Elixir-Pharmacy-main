...
import { Badge, Button, Flex, HStack, Input, InputGroup, InputLeftElement, Select, Stack, Table, Tbody, Td, Text, Th, Thead, Tr, useDisclosure } from "@chakra-ui/react";
...
const ReceivingList = () => {
  ...
  return (
    <Flex p={4} w="full" flexDirection="column">
      <Flex justifyContent="space-between" ml="2%" mb={1} mt="-10px">
        <HStack>
          <InputGroup>
            ...
            <Input value={searchValue} type="text" placeholder="Search: PO Number" onChange={(e) => searchHandler(e.target.value)} focusBorderColor="accent" />
          </InputGroup>
        </HStack>

        <HStack mr={8}>
          <Badge colorScheme="green">
            <Text color="secondary">Number of Records: {list.warehouse?.length}</Text>
          </Badge>
        </HStack>
      </Flex>

      <Flex h="735px" ml="2%" w="96%" bgColor="white" flexDirection="column" border="2px" borderWidth="5px" borderColor="secondary" justifyContent="space-between">
        <PageScrollImport>
          {!list.warehouse?.length > 0 ? (
            <NoListData />
          ) : (
            <Table variant="striped" size="sm">
              ...
              <Tbody>
                {list.warehouse?.map((l, i) => (
                  <Tr key={i}>
                    <Td>{l.ymir_PO_Number === null ? l.pO_Number : l.ymir_PO_Number}</Td>
                    <Td>{l.itemCode}</Td>
                    ...
                    <Td>
                      <Button onClick={() => returnOpener(l.id)} colorScheme="red" size="xs">
                        Return
                      </Button>
                    </Td>
                  </Tr>
      ...
      {isReturnModalOpen && <ReturnModal isOpen={isReturnModalOpen} onClose={closeReturnModal} returnId={returnId} fetchReceivingList={fetchReceivingList} />}
    </Flex>
  );
};
