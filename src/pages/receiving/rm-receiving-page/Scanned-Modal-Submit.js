...
const ScannedModalSubmit = ({ itemCodeData, code, receivingDate, lotCategory, actualGood, sumQuantity, submitRejectData, buttonChanger, receivingId, setCode }) => {
  ...
  const submitHandler = () => {
    ...
    try {
      setIsLoading(true);
      const res = apiClient
        .put(`Warehouse/ReceiveRawMaterialsInWarehouse/${firstSubmit.itemCode}`, firstSubmit)
        .then((res) => {
    ...
    <>
      {buttonChanger === true ? (
          ...
          <Box w="20%" display="none">
            <VStack spacing={0} justifyContent="center" ref={componentRef}>
              ...
              <Flex mt={2} w="90%" justifyContent="center">
                <Text fontSize="25px" fontWeight="semibold" ml={4}>
                  Raw Materials
                </Text>
              </Flex>
        ...
        <Flex justifyContent="end" mt={2} mr={2}>
          <Button size="md" onClick={onOpen} colorScheme="blue" _hover={{ bgColor: "accent" }}>
            Receive
          </Button>
        </Flex>
      )}
      ...
      <Modal isOpen={isOpen} onClose={() => {}} isCentered size="md">
        ...
        <ModalContent>
          ...
          <ModalFooter>
            <Button onClick={() => submitHandler()} colorScheme="blue" mr={3} _hover={{ bgColor: "accent" }}>
              Yes
            </Button>
