        ...
        <AccordionItem>
          ...
          <AccordionPanel>
            <Flex justifyContent="space-between">
              <FormLabel w="40%">
                Quantity
                <Input value={quantity} onChange={(e) => quantityHandler(e.target.value)} isInvalid={errors.qty} placeholder="Quantity" type="number" />
              </FormLabel>

              <FormLabel w="40%">
                Remarks
                {reasons.length > 0 ? (
                  <Select ref={remarksDisplay} onChange={(e) => remarksHandler(e.target.value)} isInvalid={errors.rms} placeholder="Select Reason">
                    {reasons?.map((reason) => (
