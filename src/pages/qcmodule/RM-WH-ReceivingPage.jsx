...
const RMWHReceivingPage = ({ fetchNotification }) => {
  ...
  return (
    <Flex p={5} w="full" flexDirection="column">
      ...
      <Flex mb={2} justifyContent="space-between">
        ...
        <HStack>
          <Badge colorScheme="green">
            <Text color="secondary">Number of Records: {whData.warehouse?.length}</Text>
          </Badge>
        </HStack>
      </Flex>
          ...
          <Table variant="striped" size="sm">
            ...
            <Tbody>
              {whData.warehouse?.map((wh) => (
                <Tr key={wh.id}>
                  {/* <Td>{wh.id}</Td> */}
                  <Td>{wh?.ymir_PO_Number === null ? wh.pO_Number : wh.ymir_PO_Number}</Td>
                  <Td>{wh.itemCode}</Td>
                  ...
                  <Td>
                    <Button onClick={() => cancelReceivingHandler(wh)} color="white" bgColor="danger" _hover={{ bgColor: "secondary", color: "white" }} size="xs">
                      Cancel
                    </Button>
                  </Td>
                </Tr>
      ...
      {isModalOpen && <ModalComponent modalData={modalData} isOpen={isModalOpen} onClose={closeModal} fetchWHReceiving={fetchWHReceiving} fetchNotification={fetchNotification} />}
