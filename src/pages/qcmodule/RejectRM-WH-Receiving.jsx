...
const RejectRMWHReceiving = ({ fetchNotification }) => {
  ...
  return (
    <Flex p={5} w="full" flexDirection="column">
      ...
      <Flex mb={2} justifyContent="space-between">
        <HStack>
          <InputGroup>
            ...
            <Input value={searchValue} onChange={(e) => searchHandler(e.target.value)} type="text" placeholder="Search: PO Number" focusBorderColor="accent" />
          </InputGroup>
        </HStack>
        <HStack>
          <Badge colorScheme="green">
            <Text color="secondary">Number of Records: {rejectData?.reject?.length}</Text>
          </Badge>
        </HStack>
      </Flex>
          ...
          <Table variant="striped" size="sm">
            ...
            <Tbody>
              {rejectData.reject?.map((rd) => (
                <Tr key={rd.id}>
                  {/* <Td>{rd.id}</Td> */}
                  <Td>{rd.ymir_PO_Number === null ? rd.pO_Number : rd.ymir_PO_Number}</Td>
                  <Td>{rd.itemCode}</Td>
                  ...
                  <Td>{rd.remarks}</Td>
                  <Td>
                    <Button onClick={() => confirmRejectedHandler(rd.qcReceivingId)} color="white" colorScheme="blue" _hover={{ bgColor: "secondary", color: "white" }} size="xs">
                      Confirm
                    </Button>
                  </Td>
                  <Td>
                    <Button onClick={() => returnRejectedHandler(rd.qcReceivingId)} color="white" colorScheme="red" _hover={{ bgColor: "secondary", color: "white" }} size="xs">
                      Return
                    </Button>
                  </Td>
                </Tr>
      ...
      {isConfirmModalOpen && (
        <ConfirmModal qcReceivingId={qcReceivingId} fetchReject={fetchReject} isOpen={isConfirmModalOpen} onClose={closeConfirmModal} fetchNotification={fetchNotification} />
      )}

      {isReturnModalOpen && (
        <ReturnModal qcReceivingId={qcReceivingId} fetchReject={fetchReject} isOpen={isReturnModalOpen} onClose={closeReturnModal} fetchNotification={fetchNotification} />
      )}
    </Flex>
  );
};
