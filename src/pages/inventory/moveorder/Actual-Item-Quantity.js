  ...
  useEffect(() => {
    if (barcodeData?.orders?.warehouseId) {
      if (barcodeData?.orders?.warehouseId != barcodeData?.warehouseId) {
        ToastComponent("Warning", `The barcode ${barcodeData?.warehouseId} will expire earlier than provided barcode ${barcodeData?.orders?.warehouseId}`, "warning", toast);
      }
    }
  }, [barcodeData]);
      ...
      <HStack justifyContent="space-between" mt={2}>
        ...
        <HStack spacing={5}>
          ...
          <Text bgColor="gray.200" border="1px" px={12} fontSize="sm">
            {barcodeData?.orders?.remaining ? barcodeData?.orders?.remaining : "No data with this barcode"}
          </Text>
        </HStack>
        <HStack spacing={5}>
          <Text bgColor="secondary" color="white" px={10} textAlign="start" fontSize="sm">
...
const AvailableBarcodeModal = ({ isOpen, onClose, availableBarcode, setWarehouseId, itemCode }) => {
  ...
  console.log("Available: ", availableBarcode);
