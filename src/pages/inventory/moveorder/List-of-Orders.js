...
export const ListofOrders = ({ orderListData, setItemCode, highlighterId, setHighlighterId, setQtyOrdered, setPreparedQty, orderId, setWarehouseId }) => {
  ...
  return (
    <VStack w="full" spacing={0} justifyContent="center" mt={10}>
      <Text w="full" fontWeight="semibold" fontSize="md" bgColor="secondary" color="white" textAlign="center">
        List of Orders
      </Text>
        ...
        <Table size="sm" variant="simple">
          ...
          <Tbody>
            {orderListData?.map((list, i) => (
              <Tr key={i} onClick={() => rowHandler(list)} bgColor={highlighterId === list.id ? "table_accent" : "none"} cursor="pointer">
                ...
                <Td>{list.orderRemarks ? list.orderRemarks : "-"}</Td>
                <Td>
                  {list.quantityOrder <= list.preparedQuantity ? <BsCheck2Circle fontSize="20px" title="Done" /> : <MdOutlinePendingActions fontSize="20px" title="Pending" />}
                </Td>
              </Tr>
            ))}
          </Tbody>
