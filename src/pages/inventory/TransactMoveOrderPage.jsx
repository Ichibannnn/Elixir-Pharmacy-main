      ...
      <Flex w="full" justifyContent="space-between" flexDirection="column">
        <Flex justifyContent="space-between" w="full">
          <HStack justifyContent="space-between" mt={5} pl={5}>
            <Text>Status:</Text>
            <Select onChange={(e) => setStatus(Boolean(Number(e.target.value)))}>
        ...
        {!status && (
          <HStack justifyContent="end" mr={10} mt={5}>
            <Button onClick={() => openTransact()} title={!deliveryDate ? "Please select a delivery date first" : ""} disabled={!deliveryDate} size="sm" colorScheme="blue">
              Transact
            </Button>
          </HStack>
        )}
