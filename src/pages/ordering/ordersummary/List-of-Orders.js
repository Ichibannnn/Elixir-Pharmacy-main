...
export const ListofOrders = ({ setDateFrom, setDateTo, dateFrom, dateTo, orderSummary, orderId, setOrderId, setItemCode, setOrderDate }) => {
  const TableHead = ["Line", "Order Date", "Item Code", "Item Description", "UOM", "Quantity Order", "Remarks", "Stock on Hand", "Difference"];
  ...
  return (
    <Flex width="full" p={5} flexDirection="column">
      <HStack spacing={2} w="40%" justifyContent="start">
        ...
        <Input onChange={(e) => setDateFrom(e.target.value)} value={dateFrom} w="full" type="date" bgColor="#fff8dc" />
        <Text fontSize="sm">To:</Text>
        <Input onChange={(e) => setDateTo(e.target.value)} value={dateTo} w="full" type="date" bgColor="#fff8dc" />
      </HStack>

      <Flex flexDirection="column" mt={3}>
        <Text textAlign="center" color="white" bgColor="secondary">
          List of Orders
        </Text>
            ...
            <Tbody>
              {orderSummary?.map((order, i) => (
                <Tr
                  ...
                  key={i}
                  cursor="pointer"
                  bgColor={orderId === order.id ? "table_accent" : "none" && order.quantityOrder > order.stockOnHand ? "pink" : "none"}
                >
                  <Td>{i + 1}</Td>
                  ...
                  <Td>{order.orderRemarks ? order.orderRemarks : "-"}</Td>
        ...
        <Text mt={4} fontSize="xs">
          Number of records: {orderSummary?.length}
        </Text>
      </Flex>
    </Flex>
  );
