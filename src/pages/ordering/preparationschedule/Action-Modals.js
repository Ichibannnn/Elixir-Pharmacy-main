        ...
        <ModalContent>
            ...
            <HStack justifyContent="center" textAlign="start">
              ...
              <VStack spacing={3.5}>
                {autofilled.map((items) => (
                  <Text w="70%" pl={2} bgColor="gray.200" border="1px" key={items}>
                    {items}
                  </Text>
                ))}
                <Input
          ...
          <ModalFooter>
            <ButtonGroup size="xs" mt={5}>
              <Button px={4} onClick={submitHandler} isLoading={isLoading} disabled={!quantitySubmit || isLoading} colorScheme="blue">
                Save
              </Button>
      ...
      <ModalContent>
        ...
        <ModalBody>
          <VStack justifyContent="center" mb={8}>
            ...
            {reasons.length > 0 ? (
              <Select onChange={(e) => remarksHandler(e.target.value)} placeholder="Please select a reason" w="65%" bgColor="#fff8dc">
                {reasons?.map((item, i) => (
        ...
        <ModalFooter>
          <Button onClick={() => cancelHandler()} disabled={!cancelRemarks} isLoading={isLoading} colorScheme="blue" mr={3} _hover={{ bgColor: "accent" }}>
            Yes
          </Button>
          <Button colorScheme="red" onClick={onClose} disabled={isLoading} isLoading={isLoading}>
            No
          </Button>
        </ModalFooter>
      </ModalContent>
...
export const ScheduleConfirmation = ({ isOpen, onClose, checkedItems, setCheckedItems, farmName, fetchOrders, setCurrentPage, currentPage, fetchNotification }) => {
      ...
      <ModalContent>
        ...
        <ModalFooter>
          <ButtonGroup size="xs" mt={8}>
            <Button px={5} colorScheme="blue" disabled={!preparationDate} onClick={submitValidate}>
              Yes
            </Button>
      ...
      <ModalContent>
        ...
        <ModalBody>
          <Text mt={5} textAlign="center">
            Are you sure you want to schedule these ({checkedItems?.length}) orders?
          </Text>
        </ModalBody>

        <ModalFooter>
          <ButtonGroup size="md" mt={10}>
            <Button px={5} colorScheme="blue" isLoading={isLoading} disabled={!preparationDate || isLoading} onClick={submitHandler}>
              Yes
            </Button>
            <Button onClick={onClose} isLoading={isLoading} disabled={isLoading} colorScheme="red">
              Cancel
