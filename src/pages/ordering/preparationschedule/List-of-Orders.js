  ...
  return (
    <Flex w="full" p={7} flexDirection="column">
      ...
      <VStack w="full" spacing={0} justifyContent="center" mt={10}>
        <Text w="full" fontWeight="semibold" fontSize="xl" bgColor="secondary" color="white" textAlign="center">
          List of Orders
        </Text>
        <PageScrollReusable minHeight="150px" maxHeight="640px">
          <Table size="sm" variant="simple">
            <Thead bgColor="secondary">
              <Tr>
                <Th>
                  <Checkbox onChange={parentCheckHandler} isChecked={stockData?.length === checkedItems?.length} disabled={!stockData?.length > 0} color="white">
                    Line
                  </Checkbox>
                </Th>
              ...
              {orders?.map((item, i) => (
                ...
                >
                  {item.stockOnHand >= item.quantityOrder ? (
                    <Td>
                      <Checkbox onChange={childCheckHandler} isChecked={checkedItems.includes(item.id)} value={item.id} color="black">
                        {i + 1}
                      </Checkbox>
                    </Td>
                  ) : (
                  ...
                  <Td>
                    <Button onClick={() => cancelHandler(item)} size="xs" colorScheme="red">
                      Cancel
                    </Button>
                  </Td>
                </Tr>
      ...
      {isEdit && <EditModal isOpen={isEdit} onClose={closeEdit} editData={editData} setCurrentPage={setCurrentPage} currentPage={currentPage} fetchOrders={fetchOrders} />}
